...
float pend_vel_roll, quad_vel_roll;
//quadrotor states
...
//desired states
float des_pend_roll = M_PI;
float des_pend_vel_roll = 0;
float des_quad_pos = 0;
float des_quad_vel = 0;
float des_quad_roll = 0;
float des_quad_vel_roll = 0;
//lqr gains calculated using MATLAB
float K_quad_pos = -2;
float K_quad_vel = -2.3786;
float K_quad_roll = 10.4515;
float K_quad_vel_roll = 0.9546;
float K_pend_roll = -1.4900;
float K_pend_vel_roll = 0.7905;
//control input
float u_roll = 0;
//timestamp
double time_stamp;
...
int main(int argc, char** argv){
  ...
  output_roll = node.advertise<std_msgs::Float32>("/swing_up/lqr_reduce_swing_roll_input", 1000);


  //open file
  ofstream myfile;
  myfile.open ("/home/marhes-lab/Desktop/Record File/LQRReduceSwing.txt", ios::trunc);
