...
float g = 9.8; // Gravity
float k = 300; //control input multiplier
float n = 300; //n*g is saturaion for control input
float energy;
int sign;
//General
double begin_time = 0;
...
int main(int argc, char** argv){
  ...
  output_des_linear_acceleration = node.advertise<std_msgs::Float32>("/swing_up/des_yaw_acc", 1000);


  //open file
  ofstream myfile;
  myfile.open ("/home/marhes-lab/Desktop/Record File/EnergieControlRotational.txt", ios::trunc);
    ...
    while (node.ok())
      {
        ...
        else if(duration > 0.5 && duration < 1.3){
          u_yaw = -10;  
        }
        else {
          energy = 0.5*Ip*vel_roll*vel_roll+(mp*g*lp*(1-cos(roll)))-2*mp*lp*g;
        ...
        //write into file
        myfile << fixed << setprecision(4) << time_stamp << "\t" << u_yaw << "\t" << roll << "\t" << vel_roll << endl;
